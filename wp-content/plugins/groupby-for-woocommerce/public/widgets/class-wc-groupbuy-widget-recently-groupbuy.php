<?php
/**
 * WooCommerce Group Buy Recently Viewed Group Buy Widget
 *
 * @category 	Widgets
 * @version 	1.0.0
 * @extends 	WP_Widget
...
class WC_groupbuy_Widget_Recently_Viewed_Groupbuy extends WP_Widget {
   ...
   *
	...
	function __construct() {
		...
		$this->woo_widget_cssclass = 'woocommerce widget_recently_viewed_groupbuy';
		$this->woo_widget_description = __( 'Display a list of recently viewed Group Buy Deals.', 'wc_groupbuy' );
		$this->woo_widget_idbase = 'woocommerce_recently_viewed_groupbuy';
		$this->woo_widget_name = __( 'WooCommerce Recently Viewed Group Buy Deals', 'wc_groupbuy' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->woo_widget_cssclass, 'description' => $this->woo_widget_description );

		/* Create the widget. */
		parent::__construct('recently_viewed_groupbuy', $this->woo_widget_name, $widget_ops);
	...
	function widget($args, $instance) {
		...
		$cache = wp_cache_get('recently_viewed_groupbuy', 'widget');
		...
		$viewed_products = ! empty( $_COOKIE['woocommerce_recently_viewed_groupbuy'] ) ? (array) explode( '|', $_COOKIE['woocommerce_recently_viewed_groupbuy'] ) : array();
		...
		$title = apply_filters('widget_title', empty( $instance['title'] ) ? __( 'Recently viewed Group Buy Deals', 'wc_groupbuy' ) : $instance['title'], $instance, $this->id_base);
		...
		wp_cache_set('recently_viewed_groupbuy', $cache, 'widget');
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['recently_viewed_groupbuy']) ) delete_option('recently_viewed_groupbuy');
	...
	function flush_widget_cache() {
		wp_cache_delete('recently_viewed_groupbuy', 'widget');
	}
